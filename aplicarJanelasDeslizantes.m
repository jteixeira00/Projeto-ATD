function [] = aplicarJanelasDeslizantes(dataLabels, signal_x, signal_y, signal_z)
    ...
    for i=1:length(dataLabels)
        ...
        Tframe = 4;                 % largura da janela de anï¿½lise em s
        Toverlap = 2;               % sobreposiï¿½ao das janelas em s
        Nframe = round(Tframe*fs);              % nï¿½mero de amostras na janela
        Noverlap = round(Toverlap*fs);          % nï¿½mero de amostras sobrepostas na janela
...
%             m_Z_frame = m_Z_frame/Nframe*2;
            ...
            % obter o mï¿½ximo da magnitude do sinal
            m_X_frame_max = max(m_X_frame);
            m_Y_frame_max = max(m_Y_frame);
            m_Z_frame_max = max(m_Z_frame);
% 
%             % encontrar os ï¿½ndices do mï¿½ximo da magnitude do sinal
